...
[vSet, xyzPoints, camPoses, reprojectionErrors] = computeDenseConstruction(images,vSet,cams);
camPoses = dispcameraposes(vSet,xyzPoints,reprojectionErrors,refPoses);
% Visualize the scene in 3D.
ptCloud = pointCloud(xyzPoints); 
dispDenseConstruction(camPoses,reprojectionErrors, ptCloud);
Y = double(xyzPoints);
threedpoints = array2table(Y);
writetable(threedpoints, '3DPointsSR.txt');
